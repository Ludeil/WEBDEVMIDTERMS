...
function syncResidentRole(PDO $pdo, int $userId): void
{
    ...
    $stmt->execute([
        'role' => $hasApproved ? 'resident' : 'applicant',
        'id' => $userId
    ]);
}
    ...
    try {
        ...
        if ($action === 'move_out') {
            ...
            if (
                !$userId ||
                !$roomId ||
                !in_array($appStatus, $appStatuses, true)
            ) {
                throw new RuntimeException(
            ...
            if (
                !$newUserId ||
                !$newRoomId ||
                !in_array($newStatus, $appStatuses, true)
            ) {
                throw new RuntimeException(
                    'Please provide valid application details.'
                );
            ...
            if (
                $oldApproved &&
                (!$newApproved || $newRoomId !== $oldRoomId)
            ) {
                restoreRoomSlot($pdo, $oldRoomId);
            }

            if (
                $newApproved &&
                (!$oldApproved || $newRoomId !== $oldRoomId)
            ) {
                consumeRoomSlot($pdo, $newRoomId);
            }
            ...
            if ($newUserId !== $oldUserId) {
                syncResidentRole($pdo, $newUserId);
            }
        ...
        header(
            'Location: room_applications.php?status=success&message=' .
        ...
        header(
            'Location: room_applications.php?status=error&message=' .
...
<div class="admin-layout">
    ...
    <main class="admin-main">
        ...
        <?php if ($message): ?>
            ...
            <div
                class="alert <?= $statusMessage === 'error' ? 'error' : 'success' ?>"
            >
                <?= h($message) ?>
            </div>
